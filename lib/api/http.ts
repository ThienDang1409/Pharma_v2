...
import { parseApiError, UnauthorizedError } from "../utils/handler/error-handler";
...
import type { ApiResponse, AuthResponseDto, RefreshTokenResponseDto } from "../types/api.types";
...
function isRefreshTokenPayload(value: unknown): value is RefreshTokenResponseDto {
  return (
    typeof value === "object" &&
    value !== null &&
...
async function refreshToken(): Promise<string | null> {
  ...
  try {
    // Call refresh endpoint - axios direct call to avoid infinite loop
    ...
    // Support both wrapped and direct refresh token response
    if (isRefreshTokenPayload(payload)) {
      // Store tokens and preserve user data
