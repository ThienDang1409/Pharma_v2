...
import { CategoryGridSkeleton } from "../common/Skeletons";
        ...
        () => informationApi.getAll(),
        {
          onSuccess: (response) => {
            ...
            const productsRoot = allCategories.find(
              (cat: Information) => cat.slug === "products" && !cat.parentId
            );

            if (productsRoot) {
              // Get level 2 categories (children of PRODUCTS)
              const level2Categories = allCategories.filter(
                (cat: Information) => cat.parentId === productsRoot._id
              );
              ...
              <div className="relative h-96">
                ...
                <div className="absolute inset-0 bg-linear-to-br from-gray-100 to-gray-200">
                  {category.image?.cloudinaryUrl ? (
                    <OptimizedImage
                      src={category.image.cloudinaryUrl}
                      alt={category.name}
                      preset="hero"
